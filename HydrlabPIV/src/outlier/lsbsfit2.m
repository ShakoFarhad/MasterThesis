...
% in the regular grid X, (excluding the middle point).
    ...
    %Shako Farhad: Added this part to make sure that idy and idx do not contain
    %negative integers or zeros or values greater than the dimensions of X. As
    %long as the sub windows and search areas are not set to be significantly 
    %bigger than the image (which does not make sense to do), then this 
    %modification does not alter the results.
    idy(idy(:)<1) = 1;
    idx(idx(:)<1) = 1;
    idy(idy(:)>I) = I;
    idx(idx(:)>J) = J;
